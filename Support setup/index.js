...
client.on('ready', async () => {
	...
	console.log(`${client.user.tag} is Ready!`)	client.user.setPresence({
	...
	//Registering Slash
		...
		if (config.enable_slash) {
				...
				const rest = new REST({ version: '9' }).setToken(process.evn.token)
		...
		try {
			...
			console.log('Successfully reloaded application (/) commands.')//Discord Client
const { Client, Intents, MessageActionRow, MessageButton } = require('discord.js')
const express = require('express');
const server = express();
...
//Bot Coded By ❀TEDDY❀#1359 for Shisui#4094 ' halloween event.
//For Support Dm Me (https://discord.com/users/893449442789830656)


client.login(process.env.token).catch(() => console.log('Invalid Token.Make Sure To Fill token in secret'))
...
//BOT IS CODED BY ❀TEDDY❀#1359  FOR Shisui#4094  in 🎃spooktoober event

//For Support Dm Me (https://discord.com/users/893449442789830656)

client.login(process.env.token).catch(() => console.log('Invalid Token.Make Sure To Fill in secret token'))
